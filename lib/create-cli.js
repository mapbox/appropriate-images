...
module.exports = (imageConfig, options) => {
  if (!imageConfig || !options || !options.inputDirectory || !options.outputDirectory) {
    throw new Error('You must provide an image config and the inputDirectory and outputDirectory options');
  }
  ...
  if (ids.length === 0 && !cli.flags.all) {
    console.log(`${chalk.red.bold('Usage error:')} You must specify image ids or use --all`);
    cli.showHelp();
  }
  ...
  generate(imageConfig, generateOptions)
    ...
    .catch(error => {
      if (Array.isArray(error) && error.every(e => e instanceof errors.UsageError)) {
